...
description: Optional advanced frontend flow for sessions that already have image generation,
  screenshot viewing, and browser verification tools available. Use only when the
  user explicitly wants an image-led concept-to-implementation workflow.
---
...
## OpenCode Starter Compatibility

This skill is not loaded by default in the starter. Copy it into
`.opencode/skills/` only when the session actually has image generation,
screenshot viewing, and browser verification tools. If the current OpenCode
session does not expose those tools, use `frontend-skill`, `taste-skill`,
`redesign-skill`, `react-best-practices`, or `shadcn` instead, then verify with
the project's normal build, tests, and manual browser checks.
